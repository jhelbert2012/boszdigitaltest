...
<%
    ...
    if(parSys.paragraphs()!=null && (parSys.paragraphs().size()<limit || limit==0)){
        if (editContext != null) {
            editContext.setAttribute("currentResource", null);
            // draw 'new' bar
            IncludeOptions.getOptions(request, true).getCssClassNames().add("section");
            %><sling:include path="*" resourceType="<%= newType %>" replaceSuffix="<%= (new Integer(limit - parSys.paragraphs().size())).toString() %>"/><%
        }
     } // end limit validation
%>Max components <%= limit %>, size <%= parSys.paragraphs().size() %>